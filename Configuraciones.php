...
function GuardarDirectores() {
    ...
    Cargando();
    $.post("Guardar.php?metodo=GuardarDirectores&directorchihuahua=" + directorchihuahua + "&directorjuarez=" +
        directorjuarez,
        function(data) {
            ValidarGuardado(data, "Los directores de guardaron correctamente",
                "Ocurrió un error al guardar los directores");
            CerrarLoader();
        });
}
...
function GuardarJefe() {
    ...
    Cargando();
    $.post("Guardar.php?metodo=GuardarJefe&puestojefe=" + puestojefe + "&nombrejefe=" +
        nombrejefe,
        function(data) {
            ValidarGuardado(data, "Los datos se guardaron correctamente",
    ...
    if (VerificarCantidad("#anio", 2000)) {
        if (VerificarCantidadMaximo("#anio", 2200)) {
            ...
            $.post("Guardar.php?metodo=GuardarNuevoPeriodo&periodo=" + periodo + "&anio=" + anio, function(data) {
                ValidarGuardado(data, "El periodo se guardó correctamente",
...
<table class="bordered hovered hovered-cell sombra table">
    <thead>
        <tr>
            <th>
                <h4 style="color:darkblue">Periodo activo</h4>
            </th>
        </tr>
        <tr>
            <th>
        ...
        echo '</div>';
        ?>
                <input type="button" class="hovered bordered hovered sombra text-shadow success" value="Guardar"
                    onclick="GuardarPeriodoActivo()">
        </tr>
        <tr>
            <th>
                <h4 style="color:darkblue">Crear periodo</h4>
            </th>
        </tr>
        <tr>
            <th>
        ...
        <tr>
            <th>
                <h4 style="color:darkblue">Director(a) por unidad</h4>
            </th>
        </tr>
        <tr>
            <th>
        ...
        ?>
                ...
                <br>
                <input type="button" class="hovered bordered hovered sombra text-shadow success" value="Guardar"
                    onclick="GuardarDirectores()">
        </tr>
        <tr>
            <th>
                <h4 style="color:darkblue">Configuración Jefe de certificación</h4>
            </th>
        </tr>
        <tr>
            <th>
                <?php
        $parametros = ConsultarRegistro("Select * from Parametros where id=1");
        $puestojefe = $parametros["PuestoJefe"];
        $nombrejefe = $parametros["NombreJefe"];
        ?>
                <b>Puesto Jefe del departamento de certificación: </b>
                <div class="input-control text">
                    <input type="text" id="puestojefe" value="<?php echo $puestojefe ?>" style="width:400px">
                </div>
                <br>
                <b>Nombre Jefe del departamento de certificación: </b>
                <div class="input-control text">
                    <input type="text" id="nombrejefe" value="<?php echo $nombrejefe ?>" style="width:400px">
                </div>
                <br>
                <input type="button" class="hovered bordered hovered sombra text-shadow success" value="Guardar"
                    onclick="GuardarJefe()">
